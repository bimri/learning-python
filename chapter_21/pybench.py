...
the version running its code or all Python versions named in a list.
'''
...
def runner(stmts, pythons=None, tracecmd=False):
    ...
    """
    print(sys.version)
    for (number, repeat, smmt) in stmts:
        number = number or defnum
        repeat = repeat or defrep   # 0=defaults

        if not pythons:
            # Run stmt on this python: API call
            # No need to split lines or quotes here
...
'''
Testing scripts use this module’s function,
passing in concrete though variable lists of statements and Pythons to be tested,
as appropriate for the usage mode desired.
'''                
